	...
	if ($resql) {
		...
		if ($num > 0) {
			while ($i < $num) {
				...
				print '<input class="checkforselect flat" type="checkbox" name="mesCasesCochees[]" value="' . $objp->contractid . '_' . $objp->reflocal . '_' . $objp->reflocataire . '_' . $objp->total . '_' . $objp->rentamount . '_' . $objp->chargesamount . '_' . $objp->vat . '_' . $objp->fk_owner . '_' . $objp->fk_soc . '"' . (($objp->reflocal && ($objp->periode == 'mensuel')) ? ' checked="checked"' : "") . '/>';
				print '</td>';
				print '</tr>' . "\n";
